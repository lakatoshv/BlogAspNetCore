...
public class CommentsServiceTests
{
    ...
    public void InsertEnumerable_WhenRepositoryThrowsException_ShouldThrowException()
    {
        ...
        _commentsRepositoryMock.Setup(x => x.Insert(It.IsAny<IEnumerable<Comment>>()))
            .Throws(new Exception("Test exception"));

        //Assert
        Assert.Throws<Exception>(() => _commentsService.Insert(newComments));
    }
    ...
    /// <summary>
    /// Async Insert Enumerable comments.
    /// Should throw exception when repository throws exception.
    /// </summary>
    [Fact]
    public async Task InsertAsyncEnumerable_WhenRepositoryThrowsException_ShouldThrowException()
    {
        ...
        _commentsRepositoryMock.Setup(x => x.InsertAsync(It.IsAny<IEnumerable<Comment>>()))
            .ThrowsAsync(new Exception("Test exception"));

        //Assert
        await Assert.ThrowsAsync<Exception>(() => _commentsService.InsertAsync(newComments));
    }
    ...
    public async Task DeleteAsyncById_WhenRepositoryThrowsException_ShouldThrowException()
    {
        //Arrange
        var comment = SetupCommentFixture().Create();
        _commentsRepositoryMock.Setup(x => x.GetByIdAsync(It.IsAny<object>()))
            .ReturnsAsync(comment);
        _commentsRepositoryMock.Setup(x => x.DeleteAsync(It.IsAny<Comment>()))
            .ThrowsAsync(new Exception("Repo fail"));

        //Assert
        await Assert.ThrowsAsync<Exception>(() => _commentsService.DeleteAsync(comment.Id));
    }
